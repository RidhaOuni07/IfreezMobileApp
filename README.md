...
Instead of using `print()`, the app now uses `AppLogger`:
...
// After
AppLogger.bluetooth('Connected to device');
AppLogger.temperature('Temperature changed', temperature);
```

**Benefits:**
- Logs only appear in debug mode (not in production)
- Structured logging with tags and levels
- Better performance (uses Flutter's optimized `debugPrint()`)
- Easy to disable/filter logs by category
- Professional logging format with prefixes